...
소비자 수요의 변동을 관리하기 위해 Contoso는 각 치즈 제조 시설의 창고에 숙성된 치즈 휠의 작은 재고를 유지합니다. 이 숙성된 휠은 왁스로 밀봉되고 저장 환경은 치즈가 완벽한 상태로 유지되도록 신중하게 제어됩니다. Contoso는 컨베이어 시스템을 사용하여 대형 왁스 밀봉 치즈 휠을 창고에서 포장 시설로 이동합니다.

과거에 Contoso는 시스템에 배치된 모든 치즈를 최대 용량으로 가공하여 포장 프로세스를 실행했습니다. 포장된 치즈의 초과량은 프로모션 제안에 사용될 수 있기 때문에 문제가 되지 않았으며 필요에 따라 추가 치즈를 재고에서 꺼낼 수 있습니다. 그러나 Contoso가 경험하고 있는 상당한 성장과 전 세계 수요로 인해 변동이 증가함에 따라 이 회사는 포장되는 치즈의 양을 관리하는 데 도움이 되는 방식으로 시스템을 자동화해야 합니다.
...
> **참고**:   **lab13-setup.azcli** 스크립트는 **bash** 셸 환경에서 실행되도록 작성되었습니다. 이것은 Azure Cloud Shell에서 가장 쉽게 실행할 수 있습니다.
...
1. 파일 선택 대화 상자에서 개발 환경을 구성할 때 다운로드한 GitHub 랩 파일의 폴더 위치로 이동합니다.
    ...
    * Allfiles
    ...
    lab13-setup.azcli 스크립트 파일은 랩 13의 설치 폴더에 있습니다.
...
1. 올바른 파일을 업로드했는지 확인하려면 다음 명령을 입력합니다.
    ...
    `ls` 명령으로 현재 디렉터리의 내용을 나열합니다. lab13-setup.azcli 파일이 나열되어 있습니다.
...
1. 설치 스크립트가 포함된 이 랩에 대한 디렉터리를 만든 다음 해당 디렉터리로 이동하려면 다음 Bash 명령을 입력합니다.
    ...
    이러한 명령은 이 랩에 대한 디렉터리를 만들고 **lab13-setup.azcli**파일을 해당 디렉터리로 이동한 다음 디렉터리를 변경하여 새 디렉터리를 현재 작업 디렉토리로 만듭니다.
...
1. Cloud Shell 도구 모음에서 **lab13-setup.azcli** 파일을 편집하려면 **편집기 열기**(오른쪽에서 두 번째 단추 - {})를 클릭합니다.

1. **파일** 목록에서 lab4 폴더를 확장하려면 **lab13**을 클릭한 다음 **lab13-setup.azcli**를 클릭합니다.     
...
1. 개발 환경에 Python 3.7이 설치되어 있는지 확인합니다.

    이 과정의 랩 3에서는 Python 3.7 설치를 비롯한 랩 환경을 준비합니다. Python이 설치되어 있지 않은 경우 랩 3의 지침을 다시 참조하세요.

    > **참고**:  현재 Azure IoT EdgeHub 개발자 도구는 Python 3.8과 호환되지 않는 docker 파이 라이브러리를 사용합니다.
...
1. **컨테이너 레지스트리 만들기** 블레이드에서 **레지스트리 이름**아래에 전역고유의 이름을 입력합니다.   
    ...
    전역고유의 이름을 제공하려면 **AZ220ACR{YOUR-ID}** 를 입력합니다. 
...
1. **SKU** 드롭다운에서 **표준**이 선택되어 있는지 확인합니다.
    ...
    Azure Container Registry는 SKU라고 하는 여러 서비스 계층에서 사용할 수 있습니다. 이러한 SKU는 Azure의 프라이빗 Docker 레지스트리의 용량 및 사용 패턴과 일치하는 예측 가능한 가격 및 여러 옵션을 제공합니다.
...
1. 대시보드에서 리소스 타일을 새로 고친 다음 **AZ220ACR{YOUR-ID}** 를 클릭합니다. 

1. 왼쪽 탐색 메뉴에서 **설정**에서 **액세스 키**를 클릭합니다.

    여기서 **관리 사용자**의 사용자 이름과 암호를 찾을 수 있습니다.  
...
1. 다음 값을 기록합니다.
    ...
    * **암호**
    ...
    기본적으로 관리자 사용자 이름은 ACR 이름과 일치합니다 - **AZ220ACR{YOUR-ID}**
...
1. 명령 프롬프트를 열고 다음 명령을 입력합니다.
    ...
    자리 표시자를 기록한 정보로 바꾸고 메시지가 표시되면 기록한 암호를 입력합니다.  예:
    ...
    > **참고**: 암호를 입력하라는 메시지는 표시되지 않습니다. 위의 명령을 입력한 후 입력하시면 됩니다. 암호를 입력하면 암호가 표시됩니다. **Enter** 키를 눌러 암호를 제출합니다.  **로그인 성공** 메시지가 표시되기까지  몇 분 정도 걸릴 수 있습니다.
...
1. 모듈에 대한 Docker 이미지 리포지토리 이름을 묻는 메시지가 표시되면 다음과 같이 자리 표시자 값을 업데이트합니다.
    ...
    기본 `localhost:5000/objectcountingmodule` 리포지토리 위치의 `localhost:5000` 부분을`az220acr{your-id}.azurecr.io`와 유사한 Azure Container Registry 서버의 이름으로 대체합니다.
...
1. Visual Studio Code가 솔루션을 만들 때까지 기다립니다.
    ...
    Visual Studio Code에서 필요한 리소스 또는 C# 확장을 로드하라는 메시지가 표시되면 **예**를 클릭합니다.
...
1. `.env` 파일 내에서 자리 표시자 값을 이전에 저장한 사용자 이름 및 암호 값으로 바꿉니다. 
    ...
    `<registry-username>` 자리 표시자를 이전에 만든 Azure Container Registry의 **레지스트리 이름**(사용자 이름)으로 바꿉니다
...
1. 배포 매니페스트의 `$edgeHub` 섹션을 확인합니다. 
    ...
    `sensorToObjectCountingModule` 경로는 `SimulatedTemperatureSensor` (`/messages/modules/SimulatedTemplaratureSensor/outputs/temperatureOutput`경로) 모듈에서 사용자 지정 `OutputCounterModule` 모듈`BrokeredEndpoint(\"/modules/ObjectCountingModule/inputs/input1\")"`경로)으로 메시지를 라우팅하도록 구성됩니다.

    `ObjectCountingModuleToIoTHub` 경로는 사용자 지정 `OutputCounterModule` 모듈에서 전송되는 메시지 (`/messages/modules/SimulatedTemperatureSensor/outputs/temperatureOutput`을 통해)를 Azure IoT Hub 서비스(`$upstream`를 통해)로 라우팅하도록 구성됩니다
...
1. 대상 플랫폼을 선택하려면 **amd64**를 클릭합니다.
    ...
    > **참고**: **Ubuntu IoT Edge** 리눅스 VM을 사용하기 때문에, `amd64` 옵션이 적절한 선택입니다. Windows VM의 경우 `windows-amd64`를 사용하고 ARM CPU 아키텍처에서 실행중인 모듈의 경우 `arm32v7` 옵션을 선택하세요.
...
1. 리소스 그룹 타일에서 **AZ-220-HUB-_{YOUR-ID}_** 를 클릭하세요.
...
1. **IoT Edge 디바이스 선택하기**을 창이 나타나면 **SimulatedDevice**를 클릭하세요.
    ...
    > **참고**: 오른쪽 아래 모서리에 **권한 없음**이 발생하면 를 실행합니다 명령 팔레트에서 `Azure IoT Edge: Set IoT Hub Connection String` 명령을 설정하여 시뮬레이터 연결 문자열을 다시 구성한 다음 를 실행합니다 명령 팔레트에서 `Azure IoT Edge: Setup IoT Edge Simulator`를 설정하고 다시 장치를 선택하려고합니다.
...
1. **탐색기** 창에서**deployment.debug.template.json**을 마우스 오른쪽 클릭을 한 다음 **시뮬레이터에서 IoT Edge 솔루션 빌드 및 실행**을 클릭하세요.
    ...
    > **참고**: Windows의 **터미널**에서 `open //./pipe/docker_engine이란 메시지를 보는 경우: 시스템이 지정된 파일을 찾을 수 없습니다.`, Docker가 시작되지 않았거나 올바르게 실행중일 수 있습니다.  Docker를 다시 시작하거나 전체 컴퓨터를 다시 시작해야 할 수도 있습니다.
...
1. 터미널 창에서 빌드 프로세스 보고를 관찰합니다.
    ...
    **IoT Edge 시뮬레이터**가 실행되면 빌드한 모듈이 터미널 창에 보고되는 메시지 출력을 보내기 시작합니다.
    ...
    SimulatedTemperatureSensor    |         12/09/2019 15:05:13> 메시지 보내기: 5, 본문: [{"machine":{"temperature":23.925331861560853,"pressure":1.3332656551145274},"ambient":{"temperature":20.69443827876562,"humidity":24},"timeCreated":"2019-12-09T15:05:13.4856557Z"}]
    ObjectCountingModule           | 수신된 메시지: 5, 본문: [{"machine":{"temperature":23.925331861560853,"pressure":1.3332656551145274},"ambient":{"temperature":20.69443827876562,"humidity":24},"timeCreated":"2019-12-09T15:05:13.4856557Z"}]
    ObjectCountingModule           | 수신된 메시지 전송
    ```
...
1. 로컬 컴퓨터의 IoT Edge 시뮬레이터에서 실행되는 `SimulatedDevice`에서 Azure IoT Hub로 전송되는 메시지를 모니터링하려면 Cloud Shell 명령 프롬프트에서 다음 명령을 입력합니다.
    ...
    위의 명령에서 `AZ-220-HUB-{YOUR-ID}` 값을 Azure IoT Hub 서비스의 이름으로 바꿔야 합니다.
...
1. 왼쪽 도구 모음에서 Visual Studio Code 디버거 보기를 열려면 **실행**을 클릭합니다.
    ...
    실행 버튼은 상단에서 아래로 네 번째 버튼이며 아이콘에 버그 셰이프가 포함되어 있습니다.
...
1. ACR(Azure Container Registry) 서비스를 열려면 리소스 그룹 타일에서 **AZ220ACR{YOUR-ID}** 를 클릭합니다.
...
1. **인증 유형**에서 **대칭 키**가 선택되었는지 확인하고 **자동 생성 키** 확인란이 선택되어 있는지 확인합니다.
...
1. **디바이스의 모듈 설정: objectcountingdevice** 블레이드의 **Container Registry 설정**에서 다음 값을 입력합니다.
    ...
    * **이름**: Azure Container Registry의 **레지스트리 이름**을 입력합니다(예: `az220acrcah191204`)
    * **주소**: Azure Container Registry 서비스의 **로그인 서버**(또는 DNS 이름)를 입력합니다(예: `az220acrcah191204.azurecr.io`)
...
1. **디바이스의 설정 모듈 : objectcountingdevice** 블레이드 상 블레이드의 하단에서 **다음** 클릭하기: **라우팅 >**.
...
1. **ObjectCountingModuleToIoTHu** 경로에 할당된 값을 검토합니다.
    ...
    > * [IoT Edge에서 모듈 배포 및 경로 설정 방법 알아보기](https://docs.microsoft.com/ko-kr/azure/iot-edge/module-composition)
